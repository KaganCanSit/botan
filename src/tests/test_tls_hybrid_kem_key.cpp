...
void roundtrip_test(Test::Result& result, Ts... kex_kem_fn) {
   ...
   result.test_eq("expected length of ciphertext is as expected",
   ...
   result.test_eq("expected shared secret has expected length",
                  decryptor.shared_key_length(0),
                  expected_shared_secret_length);
   ...
   result.test_eq("Public_Key::key_length is the maximum of its parts",
                  hybrid_public_key.key_length(),
                  expected_key_length);
...
void kex_to_kem_roundtrip(Test::Result& result,
   ...
   result.test_eq("decapsulated shared secret has expected length",
                  decaps_shared_secret.size(),
                  decryptor.shared_key_length(0));
